...
void solve(array<array<int,9>,9> state, array<array<vector<int>,9>,9> alt)
{
    ...
    if (row == -1 && column == -1)
    {
        ...
        if (!solved)
        {
            ...
            was_solved = solved;
            if (!was_solved) // Double-check before assignment
            {
                #pragma omp atomic write
                solved = true;
                sudoku = state; // Protected by atomicity of `solved`
            }
        }
    }
    else
    {
        #pragma omp parallel for shared(sudoku, solved)
        for (int i = 0; i < alt[row][column].size(); i++)
        {
            bool was_solved;
            #pragma omp atomic read
            was_solved = solved;
            if (was_solved) // Stop if another thread has solved it
                continue;
...
int main()
{
    ...
    cout << "Time taken to solve the Sudoku: " << (end_time - start_time) << " seconds.\n";
